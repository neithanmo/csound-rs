...
pub trait Handler {
    ...
    fn midi_read_cb(&mut self, buffer: &mut [u8]) -> usize;

    fn midi_write_cb(&mut self, buffer: &[u8]) -> usize;
...
impl Handler for CallbackHandler {
    ...
    fn keyboard_cb(&mut self) -> char {
        if let Some(fun) = self.callbacks.keyboard_cb.as_mut() {
            return fun();
        }
        '\0'
    }
    ...
    fn midi_read_cb(&mut self, buff: &mut [u8]) -> usize {
    ...
    fn midi_write_cb(&mut self, buff: &[u8]) -> usize {
